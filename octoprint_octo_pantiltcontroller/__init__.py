...
class Octo_pantiltcontrollerPlugin(octoprint.plugin.SettingsPlugin, octoprint.plugin.AssetPlugin, octoprint.plugin.TemplatePlugin, octoprint.plugin.SimpleApiPlugin, octoprint.plugin.StartupPlugin):
    def __init__(self):
        self.panValue = 0
        self.tiltValue = 0
    ...
    def get_assets(self):
        return dict(js=["js/octo_pantiltcontroller.js"], css=["css/octo_pantiltcontroller.css"], less=["less/octo_pantiltcontroller.less"])
    ...
    def get_update_information(self):
        return dict(
            octo_pantiltcontroller=dict(
                displayName="Pan/Tilt Controller Plugin",
                displayVersion=self._plugin_version,
                type="github_release",
                user="Waylon Wang",
                repo="OctoPrint-PanTiltController",
                current=self._plugin_version,
                # update method: pip
                pip="https://github.com/waylonwang/OctoPrint-PanTiltController/archive/{target_version}.zip",
            )
        )


__plugin_name__ = "OctoPrint Pan Tilt Controller"
__plugin_pythoncompat__ = ">=2.7,<4"


def __plugin_load__():
    global __plugin_implementation__
    __plugin_implementation__ = Octo_pantiltcontrollerPlugin()
